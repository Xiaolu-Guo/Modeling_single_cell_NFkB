function [] = scSRS_corr_heterogeneity_202410()
% For Guo et al. Figure 7 S7: single-cell stimulus response specificity
%
% tested 10/28/2024, Matlab 2024a
...
addpath('./bin/')
addpath('./revision/')
plot_vers = '_1113_test';%'_1028_equal';%'_0705';
...
if 0
    ...
    if 1 %Figure 6A S6B WT: tested 08/12/2024
        ...
        colormap_mat = [0,0,0
            1,1,1];
        color_limit_6A = [0,1];
        threshold_new = 1; % change the threshold here to get different threshold figures
...
if 0 % Figure 6D : tested on 11/16/2024
    ...
    dist_mat_IkBo = dist_mat_IkBo(:,[1,2,4,3,5,7,9,6,8,10]);
    color_limit = [0,50;0,20;0.5,1];
    threshold_vec = {0.8,0.9,1,1.1,1.2}; % {0.5,0.8,1,1.2,1.5};
    
    cell_num = 151;
    prob_vec = cell(length(threshold_vec),1);
    distinguish_mat = cell(length(threshold_vec),1);
    cell_num_95 = cell(length(threshold_vec),1);
    cell_num_99 = cell(length(threshold_vec),1);
    bar_plot_wt_cell_95prob = zeros(1,length(threshold_vec));
    bar_plot_wt_cell_99prob = zeros(1,length(threshold_vec));
    ...
    parfor i_threshold = 1:length(threshold_vec)
        

        distinguish_mat{i_threshold} = dist_mat_wt > threshold_vec{i_threshold};
        [prob_vec{i_threshold}] = prob_cell_num_SRS_dominant_effects(distinguish_mat{i_threshold},cell_num) ;


        cell_num_95{i_threshold} = find(prob_vec{i_threshold}>=0.95,1);
        cell_num_99{i_threshold} = find(prob_vec{i_threshold}>=0.99,1);

        if isempty(cell_num_95{i_threshold})
            bar_plot_wt_cell_95prob(i_threshold) = cell_num+2;
        else
            bar_plot_wt_cell_95prob(i_threshold) = cell_num_95{i_threshold} ;
        end

        if isempty(cell_num_99{i_threshold})
            bar_plot_wt_cell_99prob(i_threshold) = cell_num+2;
        else
            bar_plot_wt_cell_99prob(i_threshold) = cell_num_99{i_threshold} ;
        end
    ...
    cell_num_95 = cell(length(threshold_vec),1);
    cell_num_99 = cell(length(threshold_vec),1);
    bar_plot_red_heterogeneity_cell_95prob = zeros(1,length(threshold_vec));
    bar_plot_red_heterogeneity_cell_99prob = zeros(1,length(threshold_vec));
    ...
    parfor i_threshold = 1:length(threshold_vec)
        ...
        distinguish_mat{i_threshold} = 1-lower_diversity_conf_mat(threshold_vec{i_threshold},codon_single_cell_wt);
        [prob_vec{i_threshold}] = prob_cell_num_SRS_dominant_effects(distinguish_mat{i_threshold},cell_num) ;
        ...
        if isempty(cell_num_95{i_threshold})
            bar_plot_red_heterogeneity_cell_95prob(i_threshold) = cell_num+2;
        else
            bar_plot_red_heterogeneity_cell_95prob(i_threshold) = cell_num_95{i_threshold} ;
        end

        if isempty(cell_num_99{i_threshold})
            bar_plot_red_heterogeneity_cell_99prob(i_threshold) = cell_num+2;
        else
            bar_plot_red_heterogeneity_cell_99prob(i_threshold) = cell_num_99{i_threshold} ;
        end
    ...
    cell_num_95 = cell(length(threshold_vec),1);
    cell_num_99 = cell(length(threshold_vec),1);
    bar_plot_IkBamut_cell_95prob = zeros(1,length(threshold_vec));
    bar_plot_IkBamut_cell_99prob = zeros(1,length(threshold_vec));
    ...
    parfor i_threshold = 1:length(threshold_vec)
        ...
        distinguish_mat{i_threshold} = dist_mat_IkBo > threshold_vec{i_threshold};
        [prob_vec{i_threshold}] = prob_cell_num_SRS_dominant_effects(distinguish_mat{i_threshold},cell_num) ;
        ...
        if isempty(cell_num_95{i_threshold})
            bar_plot_IkBamut_cell_95prob(i_threshold) = cell_num+2;
        else
            bar_plot_IkBamut_cell_95prob(i_threshold) = cell_num_95{i_threshold} ;
        end

        if isempty(cell_num_99{i_threshold})
            bar_plot_IkBamut_cell_99prob(i_threshold) = cell_num+2;
        else
            bar_plot_IkBamut_cell_99prob(i_threshold) = cell_num_99{i_threshold} ;
        end
    ...
    barplot_include_mutant_all_95 = [bar_plot_wt_cell_95prob;bar_plot_IkBamut_cell_95prob];% ;bar_plot_red_heterogeneity_cell_95prob
    barplot_include_mutant_all_99 = [bar_plot_wt_cell_99prob;bar_plot_IkBamut_cell_99prob];% ;bar_plot_red_heterogeneity_cell_99prob
    ...
    saveas(gcf,strcat(savepath,'fig6_probSRS95_cell_num_1116'),'epsc');%_20cells
    ...
    set(gca,'XTickLabels',{},'YTickLabels',{})
    saveas(gcf,strcat(savepath,'fig6_probSRS99_cell_num_1116'),'epsc');%_20cells
    ...
    set(gcf, 'PaperUnits','points')
    set(gcf, 'PaperPosition', paperpos,'PaperSize', papersize,'Position',paperpos)
    bar(barplot_include_mutant_all_95')
    ylim([0,cell_num-1])
    set(gca,'XTickLabels',{},'YTickLabels',{})
    saveas(gcf,strcat(savepath,'fig6_probSRS95_cell_num_wIkBamut_1116'),'epsc');%_20cells
    ...
    set(gcf, 'PaperUnits','points')
    set(gcf, 'PaperPosition', paperpos,'PaperSize', papersize,'Position',paperpos)
    bar(barplot_include_mutant_all_99')
    ylim([0,cell_num-1])
    
    set(gca,'XTickLabels',{},'YTickLabels',{})
    saveas(gcf,strcat(savepath,'fig6_probSRS99_cell_num_wIkBamut_1116'),'epsc');%_20cells
...
if 1 % Figure 6C: tested on 08/12/2024
    ...
    [prob_vec] = prob_cell_num_SRS_dominant_effects(distinguish_mat,cell_num);
    figure(1)
    plot(1:cell_interval :cell_num,prob_vec(1:cell_interval :cell_num),'LineWidth',1);hold on
    figure(2)
    plot(1:cell_interval :cell_num,prob_vec(1:cell_interval :cell_num),'LineWidth',1);hold on

    % IkBamut
    distinguish_mat = dist_mat_IkBo > threshold_new;
    [prob_vec] = prob_cell_num_SRS_dominant_effects(distinguish_mat,cell_num);
    figure(2)
    plot(1:cell_interval :cell_num,prob_vec(1:cell_interval :cell_num),'LineWidth',1);hold on
    ...
    % lower diversity
    reduce_heterogeneity_conf_mat = lower_diversity_conf_mat(threshold_new,codon_single_cell_wt);
    ...
    xlim([0,cell_num-1])
    ylim([0,1])
    set(gca,'XTick',0:10:cell_num,'YTick',0:0.2:1)
    set(gca,'XTickLabels',{},'YTickLabels',{})
    saveas(gcf,strcat(savepath,'fig6_probSRS_cell_num_wt_vs_rh_',replace(num2str(threshold_new),'.','p'),plot_vers),'epsc');%_20cells
    close
    ...
    set(gca,'XTick',0:10:cell_num,'YTick',0:0.2:1)
    set(gca,'XTickLabels',{},'YTickLabels',{})
    saveas(gcf,strcat(savepath,'fig6_probSRS_cell_num_wt_vs_rh_vs_IkBamut_',replace(num2str(threshold_new),'.','p'),plot_vers),'epsc');%_20cells
    close
...
if 0
    ...
    if 0 % Figure 6B & S6B: tested 08/12/2024
        figure(1)
    ...
    if 0 %Figure S6A low diversity: tested 08/12/2024
        color_limit = [0,50;0,20;0.5,1];
...
function [prob_vec] = prob_cell_num_SRS_dominant_effects(distinguish_mat,maximal_cell_group_size)
% different from the verion in scSRS_corr_heterogeneity_202410.m, this
% version only calculate as specific when the specific group of cells more
% than the confusion groups
if nargin ==1
    maximal_cell_group_size = 10;
end
num_stim_pairs = 10;
N_samples = 100000;

prob_vec = -ones(1,maximal_cell_group_size);
for i_threshold = 1
    % distinguish_mat = traj_mat >= threshold(i_threshold);% confusion
    
    for i_cell_group_size = 1:maximal_cell_group_size
        
        srs_or_not = -ones(N_samples,1);
        for i_sample = 1: N_samples
            cell_ids = randperm(size(distinguish_mat,1),i_cell_group_size);
            group_distinguish_pairs = distinguish_mat(cell_ids,:);
            % group_distinguish_pairs(group_distinguish_pairs==0) = -1;
            distinguish_pairs_or_not = sum(group_distinguish_pairs,1);
            confusion_pairs_or_not = sum(~group_distinguish_pairs,1);
            srs_or_not(i_sample) = sum((distinguish_pairs_or_not-2*confusion_pairs_or_not)>=0) == num_stim_pairs;
        end
        
        prob_vec(i_threshold,i_cell_group_size) = sum(srs_or_not) /N_samples;
    end
...
function [conf_mat] = lower_diversity_conf_mat(threshold_new,codon_single_cell_wt)

conf_mat_wt = cell2mat(cellfun(@(x) confusion_mat(x,threshold_new), codon_single_cell_wt, 'UniformOutput', false));
conf_mat_wt = conf_mat_wt(:,[1,2,4,3,5,7,9,6,8,10]); % stimuli pair reorder
conf_score_wt = sum(conf_mat_wt,2);

conf_mat = zeros(size(conf_mat_wt));
cell_num_count = 0;
for cluster_num =0:10
    barplot(cluster_num+1,1) = sum(conf_score_wt == cluster_num);
    conf_mat(cell_num_count+1:cell_num_count+barplot(cluster_num+1,1),1:cluster_num) = 1;
